...
//
// Test steps:
//   1. Set AXI transfer parameters: len inside {legal num}, size=max_width
//   2. Enable data_before_addr and configure before_delay
//   3. Set aligned start address and transaction count (500)
//   4. Master VIP sends 500 write transactions
//   5. After ALL write transactions complete, read back and compare
//-----------------------------------------------------------------------------
class axi4_data_first_seq extends axi4_base_sequence;
    `uvm_object_utils(axi4_data_first_seq)

    localparam int MAX_SIZE = $clog2(`AI_AXI4_MAX_DATA_WIDTH / 8);

    rand bit [31:0] m_start_addr;
    int unsigned m_num_txns = 500;

    constraint c_addr_aligned {
        (m_start_addr & ((1 << MAX_SIZE) - 1)) == 0;
    }

    function new(string name = "axi4_data_first_seq");
        super.new(name);
    endfunction
    ...
    task body();
        ...
        `uvm_info(get_type_name(),
            $sformatf("Starting data_first_seq: start_addr=0x%08h num_txns=%0d size=%0d",
                      cur_addr, m_num_txns, MAX_SIZE),
            UVM_LOW)
